    ...
    <body class="bgc">
        ...
        <main class="container bg-light p-2">
            ...
            if($_SERVER["REQUEST_METHOD"] == "POST") {
                global $correctlyPosted;
                $correctlyPosted = true;
                if ($_POST["eMail"] == "") {
                    addAlert("Polje za e-mail račune je prazno.");
                    $correctlyPosted = false;
            ...
            ?>
            <form action=<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?> method="POST" class=<?php if ($correctlyPosted){echo "d-none";};?>>
            ...
            <?php
            if ($correctlyPosted) {
                echo "<div class=bg-success>Forma uspešno oddana</div>";
            };
            ?>
        </main>
        </div>
